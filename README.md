...
````PHP
function measure(BridgePropertyAccess $bridge)
{
    $limit = 1000000;
    $client = new Client();
    ...
    for ($i = 0; $i < $limit; $i++) {
        $bridge->setValue($client, 'name', 'ionut');
        $bridge->getValue($client, 'name');
        $bridge->isReadable($client,'email');
        $bridge->isWritable($client, 'id');
    }
...
echo measure($bridgePropertyAccess) . PHP_EOL;
$bridgePropertyAccess->setUsePool(false);
